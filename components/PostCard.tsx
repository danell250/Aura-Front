...
const PostCard: React.FC<PostCardProps> = ({ 
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (commentEmojiPickerRef.current && !commentEmojiPickerRef.current.contains(event.target as Node)) setActiveCommentEmojiPicker(null);
    };
      ...
      <div key={comment.id} className={`${isNested ? 'ml-10 mt-4 border-l-2 border-slate-100 dark:border-slate-800 pl-4' : 'mt-6'}`}>
        <div className="flex gap-3">
          <div onClick={() => onViewProfile(comment.author.id)} className="cursor-pointer shrink-0">
            <div className="w-8 h-8 rounded-full overflow-hidden bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm transition-transform hover:scale-110 flex items-center justify-center">
              {comment.author.avatarType === 'video' || (comment.author.avatar && comment.author.avatar.toLowerCase().match(/\.(mp4|webm|ogg|mov|gif|gifv)$/)) ? (
                <video src={comment.author.avatar} className="w-full h-full object-cover" autoPlay loop muted playsInline />
              ) : comment.author.avatar ? (
                <img src={comment.author.avatar} className="w-full h-full object-cover" alt="" />
              ) : (
                <span className="text-[10px] font-bold text-slate-400">{comment.author.name.charAt(0)}</span>
              )}
            </div>
          </div>
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mt-2 ml-2">
              ...
              <div className="relative">
                ...
                {isEmojiPickerActive && (
                  <div className="absolute bottom-full left-0 mb-4 z-[100] shadow-2xl rounded-3xl overflow-hidden animate-in fade-in slide-in-from-bottom-2 max-h-[40vh] md:max-h-[500px]" ref={commentEmojiPickerRef}>
                    <EmojiPicker onEmojiClick={(ed) => handleCommentEmojiClick(ed, comment.id)} theme={document.documentElement.classList.contains('dark') ? Theme.DARK : Theme.LIGHT} width={280} height={350} />
                  </div>
                )}
              </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-2 mb-6 flex-wrap">
           ...
           <div className="relative">
              ...
              {showEmojiPicker && (
                <div className="absolute bottom-full left-0 mb-4 z-[100] shadow-2xl rounded-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300 max-h-[40vh] md:max-h-[500px]" ref={emojiPickerRef}>
                  <EmojiPicker onEmojiClick={handlePostEmojiClick} theme={document.documentElement.classList.contains('dark') ? Theme.DARK : Theme.LIGHT} width={280} height={350} />
                </div>
              )}
           </div>
        </div>
          ...
          <div className="mt-8 pt-8 border-t border-slate-50 dark:border-slate-800/50 space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="flex gap-3">
              <div className="w-9 h-9 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 shrink-0 flex items-center justify-center">
                {currentUser.avatarType === 'video' || (currentUser.avatar && currentUser.avatar.toLowerCase().match(/\.(mp4|webm|ogg|mov|gif|gifv)$/)) ? (
                  <video src={currentUser.avatar} className="w-full h-full object-cover" autoPlay loop muted playsInline />
                ) : currentUser.avatar ? (
                  <img src={currentUser.avatar} className="w-full h-full object-cover" alt="" />
                ) : (
                  <span className="text-xs font-bold text-slate-400">{currentUser.name.charAt(0)}</span>
                )}
              </div>
