...
const NotificationDropdown: React.FC<NotificationDropdownProps> = ({ notifications, onClose, onRead }) => {
      ...
      <div className="max-h-96 overflow-y-auto no-scrollbar">
        {notifications.length === 0 ? (
          ...
          notifications.map(notif => (
            <div 
              key={notif.id} 
              onClick={() => notif.type !== 'connection_request' && onRead(notif.id)}
                ...
                {notif.type === 'connection_request' && !notif.isRead && (
                  <div className="mt-2 flex gap-2">
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        (window as any).handleAcceptConnection?.(notif);
                      }}
