...
const ProfileView: React.FC<ProfileViewProps> = ({ 
  ...
  const renderMedia = (url: string, type: 'image' | 'video' | undefined, className: string, isAvatar: boolean = false) => {
    ...
    const objectClass = isAvatar ? 'object-cover' : 'object-cover';
    if (isVideo) {
      return (
        <video key={`profile-video-${url}`} src={url} className={`${className} ${objectClass}`} autoPlay loop muted playsInline preload="auto" />
      );
    }
    return (
      <img key={`profile-img-${url}`} src={url} className={`${className} ${objectClass}`} alt="" loading="eager" unoptimized={isGif ? "true" : undefined} />
    );
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative h-80 overflow-hidden">
          {user.coverImage ? (
            renderMedia(user.coverImage, user.coverType, "w-full h-full")
          ) : (
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex flex-col lg:flex-row gap-6 flex-1">
                ...
                <div className="flex justify-center lg:justify-start">
                  <div className={`w-32 h-32 rounded-2xl overflow-hidden bg-white dark:bg-slate-800 p-1 shadow-lg ${user.activeGlow !== 'none' ? 'ring-4 ring-emerald-400/30' : ''}`}>
                ...
                <div className="flex-1 text-center lg:text-left">
                  ...
                  <div className="flex gap-8 justify-center lg:justify-start">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wide">Connections</div>
                    </div>
                    <div>
