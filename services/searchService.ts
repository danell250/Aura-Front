...
export class SearchService {
  ...
  static search(
    ...
    if (!filters.type || filters.type === 'all' || filters.type === 'posts') {
      results.push(...this.searchPosts(searchTerms, posts));
    }

    // Search users
    if (!filters.type || filters.type === 'all' || filters.type === 'users') {
      results.push(...this.searchUsers(searchTerms, users));
    }
   ...
   * Search within posts
   */
  private static searchPosts(searchTerms: string[], posts: Post[]): SearchResult[] {
    return posts.map(post => {
      ...
      // Search in comments
      const commentMatches = post.comments.some(comment =>
      ...
      relevance += Math.log(post.radiance + 1) * 0.1;
   ...
   * Search within users
   */
  private static searchUsers(searchTerms: string[], users: User[]): SearchResult[] {
  ...
  static getSuggestions(
    ...
    users.forEach(user => {
      ...
      if (user.handle.toLowerCase().includes(lowerQuery)) {
