    ...
    <div className="animate-in fade-in slide-in-from-bottom-6 duration-500">
      <div className="bg-white rounded-[2.5rem] p-8 mb-8 border border-slate-200 shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 mb-10">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Connections</h2>
            <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-1">
              Your Synced Network • {acquaintances.length} Active Connections
            </p>
          </div>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] p-20 text-center border border-slate-100 flex flex-col items-center">
          ...
          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest max-w-xs leading-relaxed">
            {search ? "No connections matching your search criteria." : "You haven't established any connections yet. Explore the network to find people."}
          </p>
          {!search && (
            <button 
              onClick={onBack}
